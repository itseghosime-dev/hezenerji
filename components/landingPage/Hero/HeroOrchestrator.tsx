...
export default function HeroOrchestrator({ images, contentNode, statsNode }: HeroOrchestratorProps) {
  ...
  useGSAP(
    () => {
      ...
      masterTl.fromTo(
        ...
        { y: 0, skewY: 0, autoAlpha: 1, duration: 1.2, stagger: 0.1, ease: "power4.out", delay: 0.1 }
      );
      ...
      masterTl.fromTo(
        ...
        { clipPath: "polygon(0% 0%, 100% 0%, 100% 100%, 0% 100%)", scale: 1, duration: 1.4, ease: "expo.inOut" },
        "-=0.4"
      );
      ...
      masterTl.add(() => {
        ...
        function playNextSlide() {
          ...
          const nextIndexText = nextIndex + 1 > 9 ? `${nextIndex + 1}` : `0${nextIndex + 1}`;
          ...
          tl.fromTo(".progress-line-fill", { scaleY: 0 }, { scaleY: 1, duration: SLIDE_DURATION, ease: "none" });
          tl.set(nextSlide, { zIndex: 2, clipPath: "polygon(0% 100%, 100% 100%, 100% 100%, 0% 100%)", scale: 1.15 });
          tl.set(activeSlideRef.current, { innerText: nextIndexText });
          tl.to(nextSlide, { clipPath: "polygon(0% 0%, 100% 0%, 100% 100%, 0% 100%)", scale: 1, duration: TRANSITION_DURATION, ease: "expo.inOut" });
          tl.set(slide, { zIndex: 0, clipPath: "polygon(0% 100%, 100% 100%, 100% 100%, 0% 100%)" });
  ...
  return (
    <section ref={containerRef} className="relative flex flex-col justify-between min-h-svh w-full overflow-hidden bg-slate-950 font-dmsans">
      {/* BACKGROUND */}
      <div className="absolute inset-0 z-0 overflow-hidden bg-slate-950">
        {images.map((img, i) => (
          ...
          >
            <Image src={img.src} alt={img.alt} fill className="object-cover object-center" sizes="100vw" priority={true} quality={90} />
          </div>
        ))}
        {/* OVERLAYS */}
      ...
      <div className="pagination-indicator opacity-0 absolute right-6 lg:right-12 top-1/2 -translate-y-1/2 z-20 flex flex-col items-center gap-4">
        <span ref={activeSlideRef} className="text-sm font-bold text-white transition-colors">01</span>
        ...
        <span className="text-xs font-medium tracking-widest text-white/40">0{images.length}</span>
      </div>
